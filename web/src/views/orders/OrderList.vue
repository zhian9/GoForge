      ...
      <div v-for="order in orders" :key="order.id" class="order-card">
        ...
        <div class="order-items">
          <div v-for="item in order.items" :key="item.id" class="order-item">
            <div class="item-img">
              <img :src="getItemImage(item)" :alt="item.productName||'商品'" @error="e=>(e.target as HTMLImageElement).src=thumbPlaceholderUri" />
            </div>
            <div class="item-info">
...
const getItemImage = (item:any) => {
  ...
  return img ? resolveUrl(img) : thumbPlaceholderUri
}
...
<style scoped>
.list-page { max-width:960px; margin:0 auto; padding:32px 24px; min-height:calc(100vh-64px); font-family:var(--gf-font); }
...
.tab-btn { padding:9px 22px; border-radius:var(--radius-pill); border:1px solid var(--gf-stroke); background:var(--gf-glass-1); color:var(--text-dim); font-size:13px; font-weight:500; cursor:pointer; box-shadow:var(--gf-inner-shadow-soft); transition:background .2s,border-color .2s,color .2s,transform .2s; }
...
.btn-go { padding:12px 32px;border-radius:var(--radius-pill);border:none;background:var(--gf-gradient);color:#04121a;font-size:15px;font-weight:700;cursor:pointer;box-shadow:var(--gf-inner-shadow-soft),0 10px 28px -12px var(--accent-glow); }
.btn-go:hover { filter:brightness(1.06); box-shadow:var(--gf-inner-shadow-soft),0 16px 36px -14px var(--accent-glow); }

/* Order Cards */
.orders-list { display:flex; flex-direction:column; gap:16px; }
.order-card { background:var(--gf-glass-2); -webkit-backdrop-filter:blur(var(--gf-blur)) saturate(var(--gf-saturate)); backdrop-filter:blur(var(--gf-blur)) saturate(var(--gf-saturate)); border:1px solid var(--gf-stroke); border-radius:var(--radius); box-shadow:var(--gf-inner-shadow); padding:20px 24px; transition:border-color .25s,box-shadow .3s,transform .3s; }
.order-card:hover { border-color:var(--gf-stroke-strong); transform:translateY(-2px); box-shadow:var(--gf-shadow-2),var(--gf-inner-shadow); }

/* Head */
.order-head { display:flex; justify-content:space-between; align-items:center; padding-bottom:16px; border-bottom:1px solid var(--gf-stroke); margin-bottom:8px; }
...
.s-0 { color:var(--danger); background:rgba(255,107,129,.12); }
.s-1 { color:var(--warning); background:rgba(251,191,107,.12); }
.s-2 { color:#7CC0FF; background:rgba(124,192,255,.12); }
.s-3 { color:var(--accent); background:var(--accent-dim); }
.s-4 { color:var(--success); background:rgba(74,222,155,.12); }
.s-5 { color:var(--danger); background:rgba(255,107,129,.12); }
...
.btn-review { padding:6px 14px;border-radius:var(--radius-pill);border:1px solid rgba(79,216,255,.4);background:var(--gf-glass-1);color:var(--accent);font-size:12px;cursor:pointer;box-shadow:var(--gf-inner-shadow-soft);transition:all .15s;flex-shrink:0; }
.btn-review:hover:not(:disabled) { background:var(--accent-dim); }
.btn-review.done { border-color:var(--gf-stroke);color:var(--text-dim);cursor:default;background:transparent; }

/* Footer */
.order-foot { display:flex; justify-content:space-between; align-items:center; padding-top:16px; border-top:1px solid var(--gf-stroke); }
.order-total { font-size:15px;color:var(--text-dim); }
.total-amount { font-size:22px;font-weight:700;color:var(--price);margin-left:8px; }
.order-actions { display:flex; gap:10px; }
.btn-pay,.btn-detail,.btn-cancel { padding:9px 20px;border-radius:var(--radius-pill);font-size:13px;font-weight:500;cursor:pointer;transition:all .2s; }
.btn-pay { border:none;background:var(--gf-gradient);color:#04121a;font-weight:700;box-shadow:var(--gf-inner-shadow-soft),0 8px 22px -12px var(--accent-glow); }
.btn-pay:hover { filter:brightness(1.06); box-shadow:var(--gf-inner-shadow-soft),0 14px 32px -14px var(--accent-glow); }
.btn-detail { border:1px solid var(--gf-stroke);background:var(--gf-glass-1);color:var(--text);box-shadow:var(--gf-inner-shadow-soft); }
.btn-detail:hover { border-color:var(--gf-stroke-strong);background:var(--gf-glass-2); }
...
.page-btn { width:40px;height:38px;border-radius:var(--radius-sm);border:1px solid var(--gf-stroke);background:var(--gf-glass-1);color:var(--text);font-size:16px;cursor:pointer;box-shadow:var(--gf-inner-shadow-soft);transition:all .2s; }
...
.modal-card { background:var(--gf-glass-deep);-webkit-backdrop-filter:blur(var(--gf-blur-lg)) saturate(var(--gf-saturate));backdrop-filter:blur(var(--gf-blur-lg)) saturate(var(--gf-saturate));border:1px solid var(--gf-stroke-strong);border-radius:var(--radius);box-shadow:var(--gf-shadow-3),var(--gf-inner-shadow);padding:28px;width:480px; }
...
.modal-textarea { width:100%;padding:12px;border-radius:var(--radius-sm);background:var(--gf-glass-1);border:1px solid var(--gf-stroke);color:var(--text);font-size:13px;resize:vertical;outline:none;font-family:inherit;box-sizing:border-box;box-shadow:var(--gf-inner-shadow-soft); }
.modal-textarea:focus { border-color:rgba(79,216,255,.5);box-shadow:var(--gf-inner-shadow-soft),0 0 0 3px var(--accent-dim); }
.modal-textarea::placeholder { color:var(--text-dim); }
.modal-footer { display:flex;justify-content:flex-end;gap:12px;margin-top:20px; }
.btn-cancel { padding:10px 24px;border-radius:var(--radius-pill);border:1px solid var(--gf-stroke);background:var(--gf-glass-1);color:var(--text);font-size:14px;cursor:pointer;box-shadow:var(--gf-inner-shadow-soft); }
...
.btn-save:hover:not(:disabled) { filter:brightness(1.06);box-shadow:var(--gf-inner-shadow-soft),0 16px 34px -14px var(--accent-glow); }
