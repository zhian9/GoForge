      ...
      <div class="card">
        ...
        <div class="items-list">
          <div v-for="item in order.items" :key="item.id" class="item-row">
            <div class="item-img">
              <img :src="getProductImage(item)" :alt="getProductName(item)" @error="e=>(e.target as HTMLImageElement).src=thumbPlaceholderUri" />
            </div>
            <div class="item-info">
...
/* Info Card */
.info-card { background:var(--gf-glass-2); -webkit-backdrop-filter:blur(var(--gf-blur)) saturate(var(--gf-saturate)); backdrop-filter:blur(var(--gf-blur)) saturate(var(--gf-saturate)); border:1px solid var(--gf-stroke); border-radius:var(--radius); box-shadow:var(--gf-inner-shadow); padding:24px; margin-bottom:20px; }
...
.info-value.accent { font-size:20px; font-weight:700; color:var(--price); letter-spacing:-.01em; }

/* Actions */
.actions-bar { display:flex; justify-content:flex-end; gap:12px; margin-bottom:20px; }
.btn-pay { padding:12px 40px; border-radius:var(--radius-pill); border:none; background:var(--gf-gradient); color:#04121a; font-size:15px; font-weight:700; cursor:pointer; box-shadow:var(--gf-inner-shadow-soft),0 12px 32px -14px var(--accent-glow); transition:filter .2s,box-shadow .2s,transform .2s; }
.btn-pay:hover { filter:brightness(1.06); box-shadow:var(--gf-inner-shadow-soft),0 18px 42px -16px var(--accent-glow); transform:translateY(-1px); }

/* Status Tags */
.status-tag { display:inline-block; padding:4px 14px; border-radius:100px; font-size:12px; font-weight:600; width:fit-content; }
.status-1,.pay-1,.ship-0 { color:var(--warning); background:rgba(251,191,107,.12); }
.status-2,.pay-2,.ship-1 { color:#7CC0FF; background:rgba(124,192,255,.12); }
.status-3,.pay-3,.ship-2 { color:var(--accent); background:var(--accent-dim); }
.status-4,.ship-3 { color:var(--success); background:rgba(74,222,155,.12); }
.status-5,.pay-4 { color:var(--danger); background:rgba(255,107,129,.12); }
